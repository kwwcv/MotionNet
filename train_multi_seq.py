...
args.data = '/data1/wangkewei/ThreeD_tasks/bev_nuScenes/train/'
args.use_bg_tc = True
args.use_fg_tc = True
args.use_sc = True
args.batch = 2
...
def train(model, criterion, trainloader, optimizer, device, epoch):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        loss_disp, loss_class, loss_motion, loss_bg_tc, loss_sc, loss_fg_tc = \
            compute_and_bp_loss(optimizer, device, num_future_frames[0].item(), all_disp_field_gt, all_valid_pixel_maps,
...
def background_temporal_consistency_loss(disp_pred, pixel_cat_map_gt, non_empty_map, trans_matrices):
    ...
    translate_to_global = np.array([[1.0, 0.0, -128.0], [0.0, 1.0, -128.0], [0.0, 0.0, 1.0]], dtype=np.float32)
    ...
    idx_x = np.arange(pixel_cat_maps.shape[2])
    idx_y = np.arange(pixel_cat_maps.shape[3])
    idx_x, idx_y = np.meshgrid(idx_x, idx_y, indexing='ij')
    
    mask1 = (non_empty_map.numpy()[0, 0] > 0)
    mask2 = (non_empty_map.numpy()[0, 1] > 0)
    seq1_pixel_cat_map = pixel_cat_maps[0, 0]
    seq2_pixel_cat_map = pixel_cat_maps[0, 1]

    X1 = idx_x[mask1]
    Y1 = idx_y[mask1]
    color_seq1 = seq1_pixel_cat_map[mask1].astype(np.int64)
    X2 = idx_x[mask2]
    Y2 = idx_y[mask2]
    color_seq2 = seq2_pixel_cat_map[mask2].astype(np.int64)

    fig, ax = plt.subplots(2, 2, figsize=(15, 15))
    ax[0,0].scatter(X1, Y1, c=color_seq1, s=1)
    ax[0,0].set_xlim(16, 256-16)
    ax[0,0].set_ylim(16, 256-16)
    ax[0,0].set_aspect('equal')
    ax[0,0].title.set_text('seq 1')

    ax[0,1].scatter(X2, Y2, c=color_seq2, s=1)
    ax[0,1].set_xlim(16, 256-16)
    ax[0,1].set_ylim(16, 256-16)
    ax[0,1].set_aspect('equal')
    ax[0,1].title.set_text('seq 2')

    # warp in this codes' way
    seq1_pixel_cat_map_warp1 = torch.from_numpy(seq1_pixel_cat_map.astype(np.float32))
    seq1_pixel_cat_map_warp1 = seq1_pixel_cat_map_warp1[None, None, :, :]
    seq1_pixel_cat_map_warp1 = seq1_pixel_cat_map_warp1.permute(0, 1, 3, 2)  # (batch, 1, h, w), swap x and y axis
    seq1_pixel_cat_map_warp1 = torch.flip(seq1_pixel_cat_map_warp1, dims=[2])

    grid = F.affine_grid(grid_trans_matrix[0][None, :, :], seq1_pixel_cat_map_warp1.size())
    seq1_pixel_cat_map_warp1 = F.grid_sample(seq1_pixel_cat_map_warp1, grid, mode='nearest')

    seq1_pixel_cat_map_warp1 = torch.flip(seq1_pixel_cat_map_warp1, dims=[2])
    seq1_pixel_cat_map_warp1 = seq1_pixel_cat_map_warp1.permute(0, 1, 3, 2)
    seq1_pixel_cat_map_warp1 = seq1_pixel_cat_map_warp1[0, 0].numpy()
    mask3 = (seq1_pixel_cat_map_warp1 == seq2_pixel_cat_map) * mask2
    color_seq1_warp1 = seq1_pixel_cat_map_warp1[mask3].astype(np.int64)
    ax[1,0].scatter(idx_x[mask3], idx_y[mask3], c=color_seq1_warp1, s=1)
    ax[1,0].set_xlim(16, 256-16)
    ax[1,0].set_ylim(16, 256-16)
    ax[1,0].set_aspect('equal')
    ax[1,0].title.set_text('seq 1 warp author')
    
    # warp in my way
    seq1_pixel_cat_map_warp2 = torch.from_numpy(seq1_pixel_cat_map.astype(np.float32))
    seq1_pixel_cat_map_warp2 = seq1_pixel_cat_map_warp2[None, None, :, :]
    seq1_pixel_cat_map_warp2 = seq1_pixel_cat_map_warp2.permute(0, 1, 3, 2)  # (batch, 1, h, w), swap x and y axis

    grid = F.affine_grid(torch.from_numpy(np.linalg.inv(np.vstack([grid_trans_matrix[0].numpy(), np.array([0,0,1])]))[:2, :][None, :, :].astype(np.float32)), seq1_pixel_cat_map_warp2.size())
    # grid = F.affine_grid(grid_trans_matrix[0][None, :, :], seq1_pixel_cat_map_warp2.size())
    seq1_pixel_cat_map_warp2 = F.grid_sample(seq1_pixel_cat_map_warp2, grid, mode='nearest')

    seq1_pixel_cat_map_warp2 = seq1_pixel_cat_map_warp2.permute(0, 1, 3, 2)
    seq1_pixel_cat_map_warp2 = seq1_pixel_cat_map_warp2[0, 0].numpy()
    mask4 = (seq1_pixel_cat_map_warp2 == seq2_pixel_cat_map) * mask2
    color_seq1_warp2 = seq1_pixel_cat_map_warp2[mask4].astype(np.int64)
    ax[1,1].scatter(idx_x[mask4], idx_y[mask4], c=color_seq1_warp2, s=1)
    ax[1,1].set_xlim(16, 256-16)
    ax[1,1].set_ylim(16, 256-16)
    ax[1,1].set_aspect('equal')
    ax[1,1].title.set_text('seq 1 warp ours')
